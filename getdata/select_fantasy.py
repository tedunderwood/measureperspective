# intersection_of_fantasy.py

# This script uses Library of Congress tags,
# plus a list of volumes called "fantasy"
# or "fantasy fiction"
# by the OCLC, to extract Hathi vols likely to
# be fantasy. Some further manual grooming
# will be required.
...
with open('../rawdata/oclc_fantasy_fiction.csv', encoding = 'utf-8') as f:
...
with open('../rawdata/oclc_fantasy.csv', encoding = 'utf-8') as f:
    reader = csv.DictReader(f)
    for row in reader:
        if row['wi'] in oclc:
            alreadyctr += 1
...
print(newctr, alreadyctr)
print('Percent: ' + str(alreadyctr / (newctr + alreadyctr)) )
print()
...
def isitsf(row):
    ...
    if 'Fantasy' in row['subjects']:
        sf = True
        reason = 'taggedfantasy'
...
with open('../rawdata/fantasy_intersection.csv', mode = 'w', encoding = 'utf-8') as f:
